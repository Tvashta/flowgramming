...
$('#createContestBtn').click(()=> {
    var appendString = '<br><div class="problem"><label class="w-25 float-md-left">Problem Name</label><select name="problems" class="form-control w-50" autocomplete="on">';
    ...
    appendString+='</select>';
    appendString+='<label class="w-25 float-md-left">Points</label><input type="number" class="form-control w-50">'
    appendString+='<button class="btn-sm btn btn-danger form-inline delete-btn"><i class="far fa-trash-alt"></i></button></div>'
...
$('#submit-btn').click(()=> {
    ...
    for(var i=0; i<formData.length; i++) {
        ...
        if(formData[i].name == 'contestEndTime')
            endTime = formData[i].value;
        if(formData[i].name == 'contestEndDate')
            endDate = formData[i].value;
        ...
        else if(formData[i].name == 'contestStartTime' || formData[i].name == 'contestStartDate' || formData[i].name == 'contestEndTime' || formData[i].name == 'contestEndDate')
            continue;
        else 
